...
class VideoPlayerState(context: Context, val coroutineScope: CoroutineScope) {
    ...
    private fun handleRetry(error: PlaybackException) {
        if (error.errorCode == PlaybackException.ERROR_CODE_BEHIND_LIVE_WINDOW) { runCatching { player?.seekToDefaultPosition(); player?.prepare(); player?.play() }.onFailure { onFinalPlaybackError?.invoke() }; return }
        if (retryCount < maxRetries) { isRetrying = true; isError = true; errorMessage = "جاري محاولة الاتصال تلقائياً (${retryCount + 1}/$maxRetries)..."; retryJob?.cancel(); retryJob = coroutineScope.launch { delay(3000L * (retryCount + 1)); retryCount++; refresh() } } else { isError = true; isRetrying = false; errorMessage = when (error.errorCode) { PlaybackException.ERROR_CODE_IO_BAD_HTTP_STATUS -> "المصدر غير متاح حالياً."; PlaybackException.ERROR_CODE_DECODER_INIT_FAILED -> "صيغة البث غير مدعومة على هذا الجهاز."; else -> "تعذر تشغيل هذا المصدر." }; onFinalPlaybackError?.invoke() }
    }
    private fun loadUrl() { if (url.isBlank()) return; if (!com.dstwrtv.app.core.util.NetworkUtils.isInternetAvailable(context)) { isError = true; isBuffering = false; errorMessage = "لا يوجد اتصال بالإنترنت."; return }; runCatching { val b = MediaItem.Builder().setUri(url); val lower = url.lowercase(); if (lower.contains("m3u8")) b.setMimeType(MimeTypes.APPLICATION_M3U8) else if (lower.contains("mpd")) b.setMimeType(MimeTypes.APPLICATION_MPD) else if (lower.contains(".ts")) b.setMimeType(MimeTypes.VIDEO_MP2T); player?.let { it.stop(); it.clearMediaItems(); it.setMediaItem(b.build()); it.prepare(); it.playWhenReady = true; isPlaying = true } }.onFailure { isError = true; errorMessage = "رابط البث غير صالح." } }
    fun play() { player?.let { if (!it.isPlaying) { if (it.playbackState == Player.STATE_IDLE || isError) loadUrl(); it.play(); isPlaying = true } } }
    fun pause() { player?.let { if (it.isPlaying) { it.pause(); isPlaying = false } } }
    fun seekForward() { player?.let { it.seekTo(it.currentPosition + 15000) } }; fun seekBackward() { player?.let { it.seekTo((it.currentPosition - 15000).coerceAtLeast(0)) } }; fun togglePlayPause() { if (player?.isPlaying == true) pause() else play() }; fun toggleMute() { isMuted = !isMuted; player?.volume = if (isMuted) 0f else volumeLevel }; fun refresh() { isError = false; isBuffering = true; loadUrl() }
    fun toggleResizeMode() { resizeMode = if (resizeMode == AspectRatioFrameLayout.RESIZE_MODE_FIT) AspectRatioFrameLayout.RESIZE_MODE_ZOOM else AspectRatioFrameLayout.RESIZE_MODE_FIT }
    fun updateVolume(dragAmount: Float) { volumeLevel = (volumeLevel - dragAmount * .002f).coerceIn(0f, 1f); player?.volume = volumeLevel; isMuted = volumeLevel == 0f; gestureValue = volumeLevel }
    fun updateBrightness(dragAmount: Float) { brightnessLevel = (brightnessLevel - dragAmount * .002f).coerceIn(0f, 1f); context.findActivity()?.let { val lp = it.window.attributes; lp.screenBrightness = brightnessLevel; it.window.attributes = lp }; gestureValue = brightnessLevel }
    fun showGesture(type: String) { gestureType = type; showGestureOverlay = true; gestureHideJob?.cancel() }; fun hideGestureAfterDelay() { gestureHideJob = coroutineScope.launch { delay(1000); showGestureOverlay = false; gestureType = null } }
