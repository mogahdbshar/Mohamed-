...
class InternetArchiveSourceProvider(
    ...
    override suspend fun resolve(request: SourceRequest): List<PlaybackSource> {
        ...
        val docs = runCatching { api.search("title:(\"$safeQuery\")", rows = 10).response?.docs.orEmpty() }.getOrDefault(emptyList())
        return docs.asSequence().mapNotNull { doc ->
            ...
            files.asSequence().mapNotNull { file ->
                ...
                val encoded = name.split('/').joinToString("/") { URLEncoder.encode(it, "UTF-8").replace("+", "%20") }
                PlaybackSource(
                    url = "https://archive.org/download/$identifier/$encoded",
                    label = doc.title ?: identifier,
                    quality = when { name.contains("2160", true) -> 2160; name.contains("1080", true) -> 1080; name.contains("720", true) -> 720; else -> null },
